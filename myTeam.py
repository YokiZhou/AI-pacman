...
class ReflexCaptureAgent(CaptureAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        self.weights = {'carrying': 0.0, 'successorScore': 0.0, 'getFood': 0.0
            , 'getCaplual': 0.0, 'enemyOneStepToPacman': 0.0, 'towardToGhost': 0.0,
                        'distanceToFood': 0.0,
                        'back': 0.0, 'stop': 0.0, 'eatGhost': 0.0,
                        'reverse': 0.0}
...
class OffensiveReflexAgent(ReflexCaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if myCurrentPos != self.farPoint:
            ...
            if self.aSt:
                ...
                if len(self.actionList) == 0:
                    ...
                    print
                    'eval time in A STAR for agent %d: %.4f' % (self.index, time.time() - start)
                    print
        ...
        print
        'eval time in Q-Learning for agent %d: %.4f' % (self.index, time.time() - start)
        return action
    ...
    def updateQ(self, gameState, action):
        ...
        if len(enemyGhostPosition) > 0:
            distanceToEnemyGhost = (
                min([self.getMazeDistance(myCurrentPos, ghostPos) for ghostPos in enemyGhostPosition]))
        else:
        ...
        for feature in features:
            weights[feature] = weights[feature] + (
                        self.alpha * (reward + self.discountFactor * Q[0] - currentQ) * features[feature])
    ...
    def getFeatures(self, gameState, action):
        ...
        features['enemyOneStepToPacman'] = 0.0  # detect the position of the ghost
        ...
        if len(enemyGhostPosition) > 0:
            escapeDistance = min(
                [self.getMazeDistance(successor.getAgentPosition(self.index), ghostPosition) for ghostPosition in
                 enemyGhostPosition])
            # print"empty"
        ...
        if escapeDistance < 10 or successor.getAgentPosition(self.index) == InitialPosition:
            ...
            if isScared:
                ...
                if gameState.getAgentState(self.index).isPacman and not eatMoreDot:
                    ...
                    distanceToEnemyGhost = (
                        min([self.getMazeDistance(myPosition, ghostPos) for ghostPos in enemyGhostPosition]))
                    if distanceToEnemyGhost == 1:
                        print
                        'enemy one step close'

                        features['enemyOneStepToPacman'] = -float(
                            min([self.getMazeDistance(myPosition, ghostPos) for ghostPos in enemyGhostPosition])) / (
                    ...
                    if len(minFoodDistance) > 0:
                        ...
                        if minFoodDistance[0] < 4 and minFoodDistance[
                            1] not in otherFood and distanceToEnemyGhost <= 2 and action != Directions.STOP:
                            features['back'] = 0
                            ...
                            if wallCount >= 3:
                                ...
                                if len(otherFood) > 0:
                                    minOtherFoodDistance = min(
                                        [(self.getMazeDistance(myPosition, food), food) for food in otherFood])
                                    ...
                                    print
                                    'second dot', minOtherFoodDistance[1]
                    ...
                    if eatMoreDot:
                        ...
                        if len(enemyGhostPosition) > 0:
                            ...
                            if min([self.getMazeDistance(myPosition, ghostPos) for ghostPos in enemyGhostPosition]) < 3:
                                ...
                                print
                                'run when cap almost gone'
                                features['enemyOneStepToPacman'] = -float(min(
                                    self.getMazeDistance(myPosition, ghostPos) for ghostPos in enemyGhostPosition)) / (
                                                                               walls.width * walls.height)
                                rev = Directions.REVERSE[gameState.getAgentState(self.index).configuration.direction]
                                if action == rev:
                                    features['reverse'] = 1
    ...
    def aStar(self, gameState, goal, iniPos, start):
        ...
        while not frontier.isEmpty():
            ...
            print
            'eval time in A STAR action for agent %d: %.4f' % (self.index, time.time() - start)
...
class DefensiveReflexAgent(ReflexCaptureAgent):
    ...
    def aStar(self, gameState, goal, iniPos, start):
        ...
        while not frontier.isEmpty():
            ...
            print
            'eval time in A STAR action ' \
            'for agent %d: %.4f' % (self.index, time.time() - start)
            aStarTime = time.time() - start
            if aStarTime >= 0.8:
